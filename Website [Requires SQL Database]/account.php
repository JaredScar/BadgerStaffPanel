...
if (is_logged_in()) {
    if (is_staff()) {
        // Show them their account, punishments they've given, their history, allow them to logout
        // TODO: Punishments they've given
        $userID = null;
            ...
            <div class="content-contain">
                ...
                <div id="content-view">
                    <div id="profile-info">
                        ...
                        $live = $row['live']; 
						// TODO Put your steam key replacing {KEY} 
                        ...
                        <div id="kick-info">
                            ...
                            <table class="table table-striped">
                                ...
                                while ($row = $res->fetch_assoc()) {
                                    ...
                                    $steamID = bchexdec(str_replace("steam:", "", $row[$issuedByField]));
                                    if ($steamID != 0) {
										// Replace {KEY} with your Steam API key 
                                        $steamAPI = file_get_contents('http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key={KEY}&steamids=' . $steamID);
                                        $steamData = json_decode($steamAPI)->response->players[0];
                                        echo '<td>' . $steamData->personaname . '</td>';
                                    }
...
<?php
        ...
        // Show them their account history, provide a logout button, show buttons for each punishment to be appealed (later feature)
        $userID = null;
        $stmt = $sql->prepare("SELECT `ID` FROM Users WHERE `discord` = ?;");
        $discordID = "discord:" . getUserDiscordID();
        $stmt->bind_param("s", $discordID);
        ...
        if ($res->num_rows > 0) {
            ...
            <head>
                <meta charset="UTF-8">
                <title>BadgerStaffPanel (Your Account)</title>
                <link rel="stylesheet" href="css/webReset.css"/>
                <link href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" rel="stylesheet"
                      integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh"
                      crossorigin="anonymous"/>
                            ...
                            <div id="warn-info">
                                ...
                                <table class="table table-striped">
                                    ...
                                    while ($row = $res->fetch_assoc()) {
                                        ...
                                        echo '<td><form action="" method="post"><button name="uid" value="' . $row['uid'] . '" class="appeal">Appeal?</button>'
                                            . '<input type="hidden" name="punishType" value="warn" />' . '</form></td>';
                                        echo '</tr>';
                                    }
                                    ?>
            ...
            // Show them that there discord is not connected to an account TODO
            echo '<h1>Your discord account is not connected to a BadgerStaffPanel account :(</h1>';
            ?>
<?php
        }
    }
} else {
    // Take back to login, they are not logged in
